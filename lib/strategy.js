...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    // trace-id の取得
    var traceId;
    if (req.query && req.query[ "trace-id" ]) {
      traceId = req.query[ "trace-id" ];
    } else {
      traceId = "";
    }
    ...
    this._setup(identifier, function(err, config) {
      ...
      function stored(err, state) {
        ...
        if( config.mobile ) {
          ...
          if( traceId ) params["trace-id"] = traceId;
          location = config.authorizationURL + '?' + querystring.stringify(params);
          self.redirect( location, 200 ); // Mobileの場合、302(redirect)では返さずに 200 で返すように変更する
        } else {
          location = config.authorizationURL + '?' + querystring.stringify(params);
          self.redirect(location);
        }

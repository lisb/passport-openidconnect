  ...
  if (req.query && req.query.code) {
    ...
    this._setup(identifier, function(err, config) {
      ...
      function stored(err, state) {
        ...
        var location;
        if( config.mobile ) {
          params.next = config.authorizationURL + '?' + querystring.stringify(params);
          params.os = config.mobile;
          var referrer = req.headers.referrer || req.headers.referer;
          const url = new URL( referrer );
          const search = url.search ? url.search + "&" : "?"; // ※searchが存在する場合、先頭に"?"有り
          location = url.origin + "/login-mobile-redirect" + search + querystring.stringify( params );
